...
namespace PREPROCESS {
    ...
    __global__ void preprocessCUDA(
        ...
        const int8_t* __restrict__ vox_level,
    ...
    {   
        ...
        const float vox_len = (float)(1<<(MAX_OCTREE_LEVELS - vox_level[0])); // VOXELs HAVE SAME LENGTH at a given scale.
        ...
        const float3 dir_diff = make_float3(
            vox_coner_000.x + 0.5f - cam_org.x,
            vox_coner_000.y + 0.5f - cam_org.y,
            vox_coner_000.z + 0.5f - cam_org.z,
        );
        ...
        min_block2d = {
            (uint32_t)max(0, min(((int)parallel_2d_grid.x)-1, (int)(bbox_min.x / BLOCK2D_X))),
            (uint32_t)max(0, min(((int)parallel_2d_grid.y)-1, (int)(bbox_min.y / BLOCK2D_Y)))
        };
        max_block2d = {
            (uint32_t)max(0, min(((int)parallel_2d_grid.x)-1, (int)(bbox_max.x / BLOCK2D_X))),
            (uint32_t)max(0, min(((int)parallel_2d_grid.y)-1, (int)(bbox_max.y / BLOCK2D_Y)))
        };
        int num_block2d_touched = (1 + min_block2d.y - min_block2d.y) * (1 + max_block2d.x - max_block2d.x);
        ...
        const bool debug)
    {
        ...
        RASTER_STATE::VoxelData voxData = RASTER_STATE::VoxelData::sizeAloc(chunkptr, N);
        ...
        //    |0   |0   |1   |
        // ]
        const float f_x = 0.5f * image_width / tan_fovx;
        const float f_y = 0.5f * image_height / tan_fovy;
        ...
        preprocessCUDA <<<(N + 255) / 256, 256>>> (
            ...
            vox_level.contiguous().data_ptr<int>(),
        ...
        return std::make_tuple(output_ndup_per_vox, geomBuffer);
